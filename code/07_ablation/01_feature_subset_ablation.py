...
def find_saf_feature_table(outputs_dir: Path, dataset: str, view: str) -> Optional[Path]:
    """Resolve the Phase 12 per-feature eta^2 table for SAF ranking."""
...
def ablation_for_view(
    ...
    # ── SAF: load feature-eta table for V_between ranking ──
    saf_path = find_saf_feature_table(outputs_dir, dataset, view)
    saf_enabled = saf_path is not None
    saf_df = None
    saf_qc_note = ""
    if saf_enabled:
        saf_df = pd.read_csv(saf_path, compression="gzip" if str(saf_path).endswith(".gz") else None)
        saf_required = {"feature", "var_between"}
        if not saf_required.issubset(set(saf_df.columns)):
            saf_enabled = False
            saf_qc_note = f"Missing columns: {sorted(saf_required - set(saf_df.columns))}"
        elif len(saf_df) == 0:
            saf_enabled = False
            saf_qc_note = "Empty SAF feature table"
        else:
            vb = pd.to_numeric(saf_df["var_between"], errors="coerce")
            if int(vb.isna().sum()) > 0:
                saf_enabled = False
                saf_qc_note = f"Non-finite var_between values: {int(vb.isna().sum())}"
            elif int((vb < 0).sum()) > 0:
                saf_enabled = False
                saf_qc_note = f"Negative var_between values: {int((vb < 0).sum())}"
            elif int(saf_df["feature"].astype(str).duplicated().sum()) > 0:
                saf_enabled = False
                saf_qc_note = f"Duplicate SAF feature IDs: {int(saf_df['feature'].astype(str).duplicated().sum())}"
            elif float((vb.abs() < 1e-8).mean()) > 0.50:
                saf_enabled = False
                saf_qc_note = f"Degeneracy: {float((vb.abs() < 1e-8).mean()):.1%} features have near-zero var_between"
    else:
        saf_qc_note = "Feature-eta table not found; SAF disabled for this view"
    if not saf_enabled:
        print(f"  [SAF] DISABLED for {dataset}/{view}: {saf_qc_note}")
    ...
    saf_idx_byK: Dict[int, np.ndarray] = {}
    n_at_K: Dict[int, int] = {}
    ...
    for K in k_pcts:
        ...
        # ── SAF top-k ──
        if saf_enabled:
            saf_idx, saf_names = topk_feature_indices_from_rank_table(
                saf_df, "feature", None, "var_between", feats, n_select
            )
            saf_idx_byK[K] = saf_idx
            selected_hashes[K]["saf_vbetween_topk_sha1"] = hashlib.sha1(
                (",".join(saf_names)).encode("utf-8")
            ).hexdigest()
        ...
        if save_feature_selections:
            ...
            if saf_enabled:
                add_rows("saf_vbetween_topk", saf_idx_byK[K].tolist())
    ...
    def _fold_worker(r: int, fold: int, train_idx: np.ndarray, test_idx: np.ndarray):
        ...
        for K in k_pcts:
            ...
            if saf_enabled:
                s_idx = saf_idx_byK[K]
                seed_saf = stable_int_seed(seed, dataset, view, model, r, fold, K, "SAF")
                perf_saf = train_eval_many(
                    X_train_full[:, s_idx], y_train,
                    X_test_full[:, s_idx], y_test,
                    model, task_type, metrics, seed_saf, n_classes
                )
                for m, val in perf_saf.items():
                    local_rows.append(dict(
                        repeat=r, fold=int(fold), K_pct=int(K),
                        strategy="saf_vbetween_topk", n_features=int(n_select),
                        metric=m, value=float(val)
                    ))
    ...
    for m in metrics:
        ...
        for K in k_pcts:
            ...
            # ── SAF summary stats ──
            add_stat("saf_vbetween_topk", "perf_saf")
            if {"saf_vbetween_topk", "var_topk"}.issubset(rep.columns):
                out["delta_saf_minus_var"] = repeat_level_stats(
                    (rep["saf_vbetween_topk"] - rep["var_topk"]).to_numpy()
                )
            if {"shap_topk", "saf_vbetween_topk"}.issubset(rep.columns):
                out["delta_shap_minus_saf"] = repeat_level_stats(
                    (rep["shap_topk"] - rep["saf_vbetween_topk"]).to_numpy()
                )
    ...
    for s in per_view_summaries:
        ...
        for m in metrics:
            for K_str, block in rbm[m].items():
                ...
                psf, psf_lo, psf_hi = unpack("perf_saf")
                dsf, dsf_lo, dsf_hi = unpack("delta_saf_minus_var")
                dss, dss_lo, dss_hi = unpack("delta_shap_minus_saf")
                ...
                rows.append(dict(
                    ...
                    perf_saf_mean=psf, perf_saf_ci_lo=psf_lo, perf_saf_ci_hi=psf_hi,
                    ...
                    delta_saf_var_mean=dsf, delta_saf_var_ci_lo=dsf_lo, delta_saf_var_ci_hi=dsf_hi,
                    delta_shap_saf_mean=dss, delta_shap_saf_ci_lo=dss_lo, delta_shap_saf_ci_hi=dss_hi,
    ...
    if args.scheduler == "global":
        ...
        for ds, vw in tasks:
            ...
            try:
                ...
                # ── SAF: load feature-eta table for V_between ranking ──
                saf_path = find_saf_feature_table(outputs_dir, ds, vw)
                saf_enabled = saf_path is not None
                saf_df = None
                saf_qc_note = ""
                if saf_enabled:
                    ...
                    saf_qc_note = "Feature-eta table not found; SAF disabled for this view"
                if not saf_enabled:
                    print(f"  [SAF] DISABLED for {ds}/{vw}: {saf_qc_note}")
                ...
                view_caches[key] = {
                    ...
                    "saf_idx_byK": saf_idx_byK,
                    "saf_enabled": saf_enabled,
        ...
        def _fold_worker_global(key: str, r: int, fold: int, train_idx: np.ndarray, test_idx: np.ndarray):
            ...
            saf_idx_byK = cache["saf_idx_byK"]
            saf_enabled = cache["saf_enabled"]
            n_at_K = cache["n_at_K"]
            k_pcts = cache["k_pcts"]
            metrics = cache["metrics"]
            dataset = cache["dataset"]
