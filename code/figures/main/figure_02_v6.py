...
def create_figure(outputs_dir: Path, output_path: Path):
    ...
    # Keep it low enough to avoid overlapping panel C xlabel.
    cax = fig.add_axes([0.25, 0.0025, 0.50, 0.011])
    ...
    # Put end-member captions well below tick labels to avoid collisions
    cbar.ax.text(0.0, -2.9, 'Coupled', transform=cbar.ax.transAxes,
                 fontsize=7.5, ha='left', color=COUPLED_GREEN, style='italic')
    cbar.ax.text(1.0, -2.9, 'Anti-aligned', transform=cbar.ax.transAxes,
                 fontsize=7.5, ha='right', color=ANTI_YLGREEN, style='italic')
