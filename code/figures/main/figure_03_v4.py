...
def create_figure(outputs_dir: Path, output_path: Path):
    ...
    cbar.ax.text(0.12, -3.0, "Coupled", transform=cbar.ax.transAxes,
                 fontsize=8, ha="center", color=DEEP_GREEN, fontweight="normal",
                 path_effects=[pe.withStroke(linewidth=2, foreground=BG_DARK)])
    cbar.ax.text(0.88, -3.0, "Anti-aligned", transform=cbar.ax.transAxes,
                 fontsize=8, ha="center", color=YLGREEN, fontweight="normal",
                 path_effects=[pe.withStroke(linewidth=2, foreground=BG_DARK)])
